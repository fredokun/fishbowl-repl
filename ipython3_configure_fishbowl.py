...
os.makedirs(config.ipython_dir + "/kernels/fishbowl", exist_ok=True)

if config.lisp_implementation == "sbcl":    
    KERNEL_SPEC = {
        "argv": [
            config.lisp_executable,'--non-interactive', '--load',
            "{0}/fishbowl.lisp".format(config.fishbowl_startup_def_dir),
            "{0}/src".format(config.fishbowl_startup_def_dir),
            config.fishbowl_startup_def_dir,
            '{connection_file}'],
...
kernel_spec_file = open(config.ipython_dir + "/kernels/fishbowl/kernel.json", "w") #
json.dump(KERNEL_SPEC, kernel_spec_file)
kernel_spec_file.close()
